...
const header = () => {
  ...
  return (
    <div className='relative bg-zinc-700 whitespace-nowrap mt-0'>   <div data-fcomponent="navbar" className="max-md:px-10 fixed z-[999] w-full px-20 py-8 font-['neue'] flex justify-between items-center ">
        <div className='logo'><svg width="72" height="30" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className='menu hidden max-md:flex'>
        <div onClick={toggleMenu} ><RxHamburgerMenu  className="text-2xl cursor-pointer" />
        </div>
                    </div>
                                ...
                                <div className="links flex gap-10 max-md:hidden">
                                   ...
                                   {["Services", "Our Work", "About Us","Insights","Contact"].map((item,index)=>(
                                    <a key={index} className={`capitalize text-lg font-regular font-light ${index === 4 && "ml-32"}`}>{item}</a>
                                   ))} 
                                </div>
                                ...
                                {isOpen && (
        <div className="links  z-[99] absolute top-20 left-0 bg-zinc-100 w-full gap-[1px] flex  flex-col items-start md:hidden ">
          {['Services', 'Our Work', 'About Us', 'Insights', 'Contact'].map((item, index) => (
            <a key={index} className="capitalize text-lg text-black font-[neue] hover:text-zinc-100 hover:bg-zinc-800 w-full py-2 font-medium px-3">
              {item}
            </a>
          ))}
        </div>
